                        ...
                        <div class="col card-form__body card-body">
                            ...
                            @foreach($locales as $key => $locale)
                                <div class="form-group">
                                    ...
                                    <input id="name:{{ $key }}" name="name:{{ $key }}" dir="auto" type="text" class="form-control" placeholder="{{ __("$locale Name") }}" value="{{ old("name:$key", $load->getTranslation($key)->name) }}">
                                </div>
                            @endforeach
                                ...
                                <div class="form-group col">
                                    ...
                                    <select id="truck_id" name="truck_id" data-toggle="select" class="form-control select2 truck_id">
                                        ...
                                        @forelse($trucks as $truck)
                                            <option value="{{ $truck->id }}" {{ old('truck_id', $load->truck_id) == $truck->id ? 'selected' : '' }}> {{ $truck->name }} </option>
                                        @empty
