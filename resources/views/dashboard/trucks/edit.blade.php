            ...
            $('#mainphoto').dropzone({
                ...
                init: function () {
                    @if(!empty(old('image')))
                        ...
                        $('.dz-progress').remove();
                    @else
                        var mock = { name: '{{ $truck->name }}', size: 2};
                        this.emit('addedfile', mock);
                        this.emit('thumbnail', mock, '{{ $truck->thumbImage }}');
                        this.emit('complete', mock);
                        $('.dz-progress').remove();
                    @endif
                ...
                <form method="post" action="{{ route('trucks.update', $truck->id) }}">
                    ...
                    @foreach($locales as $key => $locale)
                        <div class="form-group">
                            ...
                            <input id="name:{{ $key }}" name="name:{{ $key }}" dir="auto" type="text" class="form-control" placeholder="{{ __("$locale Name") }}" value="{{ old("name:$key", $truck->getTranslation($key)->name) }}">
                        </div>
                    @endforeach


                    <div class="form-group">
                        <input class="image_name" type="hidden" name="image" value="{{ $truck->image }}">
                    </div>
